...
Begin VB.Form Frm604Mx 
   BackColor       =   &H80000005&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "成品退货单(Frm604Mx)"
   ClientHeight    =   4845
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6870
...
With FrmSectList
    ...
    .W_Select_Data = " select  a.order_no as 制单编号,mtr_Amt as 订单数量," & _
                    ...
                    "  from order_outbar_amt  a inner join  mmsp011 b on a.order_no=b.order_no      " & _
                    "       and a.outbar_Amt>0 " & _
                    "  order by a.order_no "
...
'检查是否已经有入库
Set W_RS = open_RS(" select outbar_amt from order_outbar_amt where order_no='" & Trim(order_no.Text) & "' and outbar_amt>0  ")
If W_RS.EOF Then
    MsgBox "该订单并没有出货,无法退货! ", 64, "提示信息"
    Set W_RS = Nothing
    Exit Function
Else
    If Val(Mtr_Amt.Text) > W_RS!outbar_amt Then
        MsgBox "退货数超过出货数量,不允许退货!超过数量:" & Val(Mtr_Amt.Text) - W_RS!outbar_amt & "  ", 64, "提示信息"
        Exit Function
    End If
End If
