...
Begin VB.Form Frm602Mx 
   BackColor       =   &H80000005&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "成品出库单(Frm602Mx)"
   ClientHeight    =   4800
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6870
...
Set w_rs = open_RS(" select order_no from order_inbar_amt where order_no='" & Trim(order_no.Text) & "' and inbar_amt>=" & Val(Mtr_Amt.Text) & "  ")
If w_rs.EOF Then
    MsgBox "累计入库数量不够出库数量! ", 64, "提示信息"
    Set w_rs = Nothing
    Exit Function
End If
