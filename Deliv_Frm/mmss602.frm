...
Begin VB.Form mmss602 
   ...
   End
End
Attribute VB_Name = "mmss602"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
 '***********************************************
'*程序名称: 成品入/出库单 (mmss602)
'*编写日期:
'*制作人员:
'*修改日期:
'*修改人员:
...
'TDBGrid相关
Dim Gridc_602(127) As Grid_Data '存放 Grid 属性值
...
Public Sub Form_Activate()
    '当窗口激活时,刷新TDBGrid
    Call GetVSGridSetting("mmss602", "TDBGrid1", Gridc_602, g_CON_IniFile4)
    Row_Height = Gridc_602(0).Grid_RowHeight
    Call readactive
    Call RefreshGrid
End Sub
...
Private Sub Form_Load()
    ...
    'MDI子窗口按钮权限设订
    C_Off_Add = lopcheck("A", "602", G_User_ID)
    C_Off_Edit = lopcheck("U", "602", G_User_ID)
    C_Off_Delete = lopcheck("D", "602", G_User_ID)
    C_Off_View = lopcheck("V", "602", G_User_ID)
    C_Off_Print = lopcheck("P", "602", G_User_ID)
    C_Off_Save = lopcheck("S", "602", G_User_ID)
...
Private Sub readactive()
    ...
    '存储TDBGRID 的属性
    Call SetVSGridSetting(TDBGrid1, Gridc_602)
    ...
    If LockRecord("mmst531", "Inv_no = '" & Trim(Inv_No.Text) & "'") Then
        ...
        With Frm602Mx
            .UpdateMode = 1
             ...
             Set .CallForm = Me
             .Inv_No = Trim(Inv_No.Text)
             .Bar_No.Text = Adodc1.Recordset!Bar_Name
...
Private Sub Form_Unload(Cancel As Integer)
    '退出时，存储 TDBGrid 属性
    Call SaveGridSetting("mmss602", "TDBGrid1", Gridc_602, g_CON_IniFile4)
...
Private Function check_ok() As Boolean
    ...
    If Inv_Style.Text = "" Then
        ...
        Exit Function
    Else
        If Inv_Style.Text <> "正常出库" And Inv_Style.Text <> "重工出库" Then
            MsgBox "请选择出库类型.", vbExclamation, g_CON_CTitle
...
Private Sub TDBGrid1_AfterUserResize(ByVal Row As Long, ByVal Col As Long)
    If Col > 0 Then
        If ColIndex > Gridc_602(0).Grid_Columns Then
            Cancel = 1
        Else
            If UCase(Mid(Gridc_602(Col - 1).Grid_Visible, 1, 1)) = "F" Or Gridc_602(Col - 1).Grid_Visible = "" Then
                Cancel = 1
            Else
                Gridc_602(Col - 1).Grid_Width = TDBGrid1.ColWidth(Col)
            End If
        End If
    End If
    ...
    If Row >= 0 Then
        ...
        Gridc_602(0).Grid_RowHeight = TDBGrid1.RowHeight(Row)
...
Private Sub TDBGrid1_BeforeMouseDown(ByVal Button As Integer, ByVal Shift As Integer, ByVal X As Single, ByVal Y As Single, Cancel As Boolean)
    ...
    If X > TDBGrid1.Left And Y < Row_Height Then
        ...
        '存储 TDBGrid 属性
        Call SaveVSGridSetting("mmss602", "TDBGrid1", Gridc_602, g_CON_IniFile4)
        
        '调用 TDBGrid 属性设定
        With mmss_set
            Set .Parent_form = mmss602
            .Get_FormName = "mmss602"
...
Set Tmp_Rb = open_RS("select '" & P_line_No.Text & "' as p_line_no,b.mag_man,b.check_man,b.bar_man," & _
                        " case when b.inv_style='正常出库' then 'V' else '' end as inv_style1, case when b.inv_style='重工出库' then 'V' else '' end as inv_style2," & _
    ...
    If C_Print Then
        C_Print = False
        Call PrintRpt(Tmp_Rb, "mmsr6021", "P")
    End If
    
    If C_View Then
        C_View = False
        Call PrintRpt(Tmp_Rb, "mmsr6021", "V")
    End If
...
'    '生产线
    ...
    '出入库种类
    Inv_Style.Clear
    Inv_Style.AddItem "正常出库"
    Inv_Style.AddItem "重工出库"
End Sub
