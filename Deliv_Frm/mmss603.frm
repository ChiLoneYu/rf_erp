...
Begin VB.Form mmss603 
   ...
   End
End
Attribute VB_Name = "mmss603"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
 '***********************************************
'*程序名称: 成品入/出库单 (mmss603)
'*编写日期:
'*制作人员:
'*修改日期:
'*修改人员:
...
'TDBGrid相关
Dim Gridc_603(127) As Grid_Data '存放 Grid 属性值
...
Public Sub Form_Activate()
    '当窗口激活时,刷新TDBGrid
    Call GetVSGridSetting("mmss603", "TDBGrid1", Gridc_603, g_CON_IniFile4)
    RoW_Height = Gridc_603(0).Grid_RowHeight
    Call readactive
    Call RefreshGrid
End Sub
...
Private Sub Form_Load()
    ...
    'MDI子窗口按钮权限设订
    C_Off_Add = lopcheck("A", "603", G_User_ID)
    C_Off_Edit = lopcheck("U", "603", G_User_ID)
    C_Off_Delete = lopcheck("D", "603", G_User_ID)
    C_Off_View = lopcheck("V", "603", G_User_ID)
    C_Off_Print = lopcheck("P", "603", G_User_ID)
    C_Off_Save = lopcheck("S", "603", G_User_ID)
...
Private Sub readactive()
    ...
    '存储TDBGRID 的属性
    Call SetVSGridSetting(TDBGrid1, Gridc_603)
...
Private Sub Form_Unload(Cancel As Integer)
    '退出时，存储 TDBGrid 属性
    Call SaveGridSetting("mmss603", "TDBGrid1", Gridc_603, g_CON_IniFile4)
...
Private Function check_ok() As Boolean
    ...
    If Inv_Style.Text = "" Then
        ...
        Exit Function
    Else
        If Trim(Inv_Style.Text) <> "正常出库" And Trim(Inv_Style.Text) <> "重工出库" Then
            MsgBox "请选择入库类型.", vbExclamation, g_CON_CTitle
...
Private Sub TDBGrid1_AfterUserResize(ByVal Row As Long, ByVal Col As Long)
    If Col > 0 Then
        If ColIndex > Gridc_603(0).Grid_Columns Then
            Cancel = 1
        Else
            If UCase(Mid(Gridc_603(Col - 1).Grid_Visible, 1, 1)) = "F" Or Gridc_603(Col - 1).Grid_Visible = "" Then
                Cancel = 1
            Else
                Gridc_603(Col - 1).Grid_Width = TDBGrid1.ColWidth(Col)
            End If
        End If
    End If
    ...
    If Row >= 0 Then
        ...
        Gridc_603(0).Grid_RowHeight = TDBGrid1.RowHeight(Row)
...
Private Sub TDBGrid1_BeforeMouseDown(ByVal Button As Integer, ByVal Shift As Integer, ByVal X As Single, ByVal Y As Single, Cancel As Boolean)
    ...
    If X > TDBGrid1.Left And Y < RoW_Height Then
        ...
        '存储 TDBGrid 属性
        Call SaveVSGridSetting("mmss603", "TDBGrid1", Gridc_603, g_CON_IniFile4)
        
        '调用 TDBGrid 属性设定
        With mmss_set
            Set .Parent_form = mmss603
            .Get_FormName = "mmss603"
...
Private Sub sele_data()
    ...
    '清除打印数据表
    G_PrintDb.Execute "DELETE * FROM mmsr6031"
    Set W_Print = G_PrintDb.OpenRecordset("SELECT * FROM mmsr6031")
'    On Error Resume Next
    '选取数据
    With W_Print
    ...
    If C_Print Then
        C_Print = False
        Call print_rpt(G_MDIForm.Rpt1, "mmsr6031", "P")
    End If
    
    If C_View Then
        C_View = False
        Call print_rpt(G_MDIForm.Rpt1, "mmsr6031", "V")
    End If
