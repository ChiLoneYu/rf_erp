...
For i = 2 To tvwDB.Nodes.Count
    If tvwDB.Nodes(i).Checked Then
       If Not W_TmpData(i - 2).Rights Then
           G_Con.Execute ("Insert into mmst907(User_list,Inv_list) values('" & User_List & "','" & W_TmpData(i - 2).D_List & "')")
       End If
    Else
       If W_TmpData(i - 2).Rights Then
           G_Con.Execute ("delete from  mmst907 where user_list='" & User_List & "' and inv_list='" & W_TmpData(i - 2).D_List & "'")
       End If
    End If
Next
...
M = 0
tvwDB.Nodes.Clear
Set Nodx = tvwDB.Nodes.Add(, , "mMENU", "审核消息反馈")
...
W_rs.Open "select user_list from mmst907 where user_list=" & User_List & " And inv_list='" & Inv_list & "' ", G_Con
