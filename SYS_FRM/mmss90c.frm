...
'定义窗体打开变量
Dim Gridc_90c(127) As Grid_Data '存放 Grid 属性值
...
If c_add Then
   With FrmSectList
'          .Quer_status = True
         .W_Select_Data = " select type_name as 产品类别,mtr_no as 物料编号,mtr_name as 物料名称,mtr_dim as 物料规格," & _
                                 " color_name as 颜色,unit_name as 单位,bar_no as 默认仓别 " & _
                          " from mmsp611 " & _
...
If c_add = True Then
    ...
    Else
        ...
        '判断 库存
         ...
         If MsgBox("料号冻结后，所有相关此物料的单据将限制操作！", vbYesNo + vbDefaultButton2 + vbInformation, g_CON_CTitle) = vbNo Then
    '                Call UnLockRecord("mmst90c", "Mtr_No='" & Mtr_No.Text & "'")
            mtr_no.SetFocus
            check_ok = False
            Exit Function
