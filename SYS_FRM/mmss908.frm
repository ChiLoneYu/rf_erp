...
Begin VB.Form mmss908 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.TextBox Supl_Name 
         ...
         Width           =   2175
      End
      Begin VB.CommandButton Cmd_Supl_Brow 
         Caption         =   "..."
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   9.75
...
'***********************************************
'定义欲打开的数据库及数据表名称
Dim St_908 As New ADODB.Recordset
...
'定义窗体打开变量
Dim Gridc_908(127) As Grid_Data '存放 Grid 属性值
Dim Row_Height As Double        'Grid 高度变量


Private Sub Cmd_Supl_Brow_Click()
If c_add Then
    With FrmSuplList
        .G_Supl_Filter = " mmst211.Supl_no like '" & Trim(Old_Supl_No.Text) & "%'  "
        .Show vbModal
        If .Supl_No <> "" Then
            Old_Supl_No.Text = .Supl_No
            Supl_Name.Text = .Supl_Name
         
        End If
        New_Supl_No.SetFocus
    End With
End If
End Sub
...
'当窗口激活时,刷新TDBGrid
Call GetVSGridSetting("mmss908", "TDBGrid1", Gridc_908, g_CON_IniFile9)
Row_Height = Gridc_908(0).Grid_RowHeight
Call readactive
'刷新表格
Call RefreshGrid
TDBGrid1.Col = 1
...
Call load_picture
'将窗口置中
Call CenterWindow(mmss908, sys_main)
...
'MDI子窗口按钮权限设订
c_off_add = lopcheck("A", "908", G_User_ID)
c_off_edit = lopcheck("U", "908", G_User_ID)
c_off_delete = lopcheck("D", "908", G_User_ID)
...
If c_add = True Or Adodc1.Recordset.EOF Or Adodc1.Recordset.BOF Then
    ...
    Remark.Text = ""
Else
    Old_Supl_No.Text = St_908!Old_Supl_No
    Supl_Name.Text = NullSetValue(St_908!Supl_Name, "")
    New_Supl_No.Text = NullSetValue(St_908!New_Supl_No, "")
    Remark.Text = NullSetValue(St_908!Remark, "")
End If
...
If Not (c_add Or c_edit) Then
    Supl_Name.Locked = True

Else
    Supl_Name.Locked = False

End If
If c_edit Then
    Old_Supl_No.Locked = True
Else
    Old_Supl_No.Locked = False
End If
End Sub
'刷新表格
...
Set St_908 = Open_Recordset(" Select Old_Supl_No  ,Supl_Name,New_Supl_no,remark,upd_name,upd_date " & _
                            " FROM Supl_Replace  ORDER BY Old_Supl_No ")



'设置tdbgrid1的数据来源
Set Adodc1.Recordset = St_908

Call SetVSGridSetting(TDBGrid1, Gridc_908)
...
Select Case p_choice
    ...
    Case "N"            '取消
        ...
        If c_edit Or c_delete Then
            Call UnLockRecord("Supl_Replace", "Old_Supl_No='" & Old_Supl_No.Text & "'")
        End If
        c_add = False
        c_edit = False
        c_delete = False
    ...
    Case "A"             '增加
        c_add = True
        Call readshow
        Old_Supl_No.SetFocus
        TDBGrid1.Enabled = False
        
    Case "U"             '修改
        '加锁
        If LockRecord("Supl_Replace", "Old_Supl_No='" & Old_Supl_No.Text & "'") Then
            W_Row = TDBGrid1.Row
            W_col = TDBGrid1.Col
    ...
    Case "D"             '删除
        '加锁
        If LockRecord("Supl_Replace", "Old_Supl_No='" & Old_Supl_No.Text & "'") = True Then
            ...
            '删除记录
            G_Con.Execute "DELETE FROM Supl_Replace WHERE Old_Supl_No='" & Trim(Old_Supl_No.Text) & "'"
            c_delete = False
            '刷新数据
...
'新增时判断
If c_add = True Then
    If Trim(Old_Supl_No.Text) = "" Then
        MsgBox "请输入旧厂商编号", 64, "提示信息"
        Old_Supl_No.SetFocus
        check_ok = False
        Exit Function
    Else
        ...
        W_Tmp.Open "select Supl_No from mmst211 where Supl_No = '" & Trim(Old_Supl_No.Text) & "'", G_Con, adOpenForwardOnly
        If W_Tmp.EOF Then
            MsgBox "旧厂商编号不存在,请重新确认!", 64, "提示信息"
            Old_Supl_No.SetFocus
            check_ok = False
            Set W_Tmp = Nothing
            Exit Function
...
If Trim(New_Supl_No.Text) = Trim(Old_Supl_No.Text) Then
    MsgBox "新旧厂商编号都相同,何必修改啊!", 64, "提示信息"
    New_Supl_No.SetFocus
    check_ok = False
    Exit Function
End If
...
'新增和修改时判断
If New_Supl_No.Text = "" Then
    MsgBox "请输入新厂商编号", 64, "提示信息"
    New_Supl_No.SetFocus
    check_ok = False
    Exit Function
Else
    ...
    '判断厂商名称是否重复
    W_Tmp.CursorLocation = adUseClient
    W_Tmp.Open "select Supl_No from mmst211 where Supl_No= '" & Trim(New_Supl_No.Text) & "' ", G_Con, adOpenForwardOnly
    
    If W_Tmp.EOF = False Then
        MsgBox "新厂商编号已经存在!", 64, "提示信息"
        New_Supl_No.SetFocus
        Set W_Tmp = Nothing
        check_ok = False
        Exit Function
...
If c_add = True Then
    With St_908
        .AddNew
        !Old_Supl_No = UCase(Trim(Old_Supl_No.Text))
        !New_Supl_No = UCase(Trim(New_Supl_No.Text))
        
        !Supl_Name = Trim(Supl_Name.Text)
    ...
    Set Tmp_Rb = Open_Recordset("select * From Supl_List")
       
    Do Until Tmp_Rb.EOF
        
        G_Con.Execute "update " & Tmp_Rb!Table_Name & " set " & Tmp_Rb!Field_name & "='" & Trim(New_Supl_No.Text) & "' Where " & Tmp_Rb!Field_name & "='" & Trim(Old_Supl_No.Text) & "'"
        Tmp_Rb.MoveNext
    Loop
    Set Tmp_Rb = Nothing
...
'退出时，存储 TDBGrid 属性
Call SaveGridSetting("mmss908", "TDBGrid1", Gridc_908, g_CON_IniFile9)
...
'各控件的相关事件
Private Sub Old_Supl_No_KeyPress(KeyAscii As Integer)
'控制输入为字母或数字
If c_add Then
...
'移动COl改变宽度
If Col > 0 Then
    If Col > Gridc_908(0).Grid_Columns Then
        Cancel = 1
    Else
        If UCase(Mid(Gridc_908(Col - 1).Grid_Visible, 1, 1)) = "F" Or Gridc_908(Col - 1).Grid_Visible = "" Then
            Cancel = 1
        Else
            Gridc_908(Col - 1).Grid_Width = TDBGrid1.ColWidth(Col)
        End If
    End If
End If
...
If Row >= 0 Then
    ...
    Gridc_908(0).Grid_RowHeight = TDBGrid1.RowHeight(Row)
...
If X > 0 And Y < Row_Height Then
    ...
    '存储 TDBGrid 属性
    Call SaveVSGridSetting("mmss908", "TDBGrid1", Gridc_908, g_CON_IniFile9)
    
    '调用 TDBGrid 属性设定
    With mmss_set
        Set .Parent_form = mmss908
        .Get_FormName = "mmss908"
